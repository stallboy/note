...
19 世纪 80 年代，美国谜题家 Sam Loyd 提出了一个著名的悬赏问题：从标准状态出发，只交换 14 和 15 的位置，能否通过合法操作将其复原？他为此悬赏 $1000。
...
答案是否定的。1879 年，数学家 William Woolsey Johnson 和 William E. Story 在 *American Journal of Mathematics* 上发表论文，首次给出了 15-puzzle 可解性的完整刻画：一个状态可解，当且仅当其对应的置换的奇偶性与空格位置满足一个简单的不变量条件。
...
**复合运算约定**：全文采用**右复合**，即 $\sigma\tau$ 表示先作用 $\sigma$、再作用 $\tau$。确切地说，$(\sigma\tau)(i) = \tau(\sigma(i))$。
...
长度为 2 的轮换称为**对换**（transposition），形如 $(a\ b)$，其效果是交换 $a$ 和 $b$ 并固定其余元素。
...
对于右复合 $\sigma\tau$（$\tau = (a\ b)$）：右复合交换的是序列中值 $a$ 和 $b$ 的位置（即值 $a$ 所在的位置 $p$ 和值 $b$ 所在的位置 $q$ 上的元素互换），这恰是上述分析的情形。$\square$
...
先算 $(a\ c\ b)$：$a \to c$，$c \to b$，$b \to a$，其余固定。

再右复合 $(a\ c\ d)$（先执行 $(a\ c\ d)$ 再执行 $(a\ c\ b)$）：

对 $a$：$(a\ c\ d)$ 把 $a \to c$，然后 $(a\ c\ b)$ 把 $c \to b$。结果 $a \to b$。

对 $b$：$(a\ c\ d)$ 固定 $b$，然后 $(a\ c\ b)$ 把 $b \to a$。结果 $b \to a$。

对 $c$：$(a\ c\ d)$ 把 $c \to d$，然后 $(a\ c\ b)$ 固定 $d$。结果 $c \to d$。

对 $d$：$(a\ c\ d)$ 把 $d \to a$，然后 $(a\ c\ b)$ 把 $a \to c$。结果 $d \to c$。

其余元素固定。因此 $(a\ c\ b)(a\ c\ d) = (a\ b)(c\ d)$，这是两个 3-轮换的乘积。
...
**合法操作**：若空格在位置 $p$，位置 $q$ 与 $p$ 相邻，则可以将空格与位置 $q$ 上的滑块交换。这在置换层面的效果是
...
> **归约命题**：空格在位置 16 时，任意满足 $\sigma(16)=16$ 的偶置换 $\sigma$（即 $\sigma$ 固定位置 16，且限制在 $\{1,\ldots,15\}$ 上为偶置换）都可以通过合法操作实现。

设 $G$ 为空格在位置 16 时所有可实现的置换的集合。$G$ 是 $S_{16}$ 的一个子群（因为两条以位置 16 为起止的操作序列可以首尾相接，逆操作也存在）。

由不变量（推论 11.2），$G \subseteq \{σ \in S_{16} : \sigma(16)=16,\ \operatorname{sgn}(\sigma)=+1\}$，后者同构于 $A_{15}$。以下证明 $G \supseteq A_{15}$。

**第二步：2×2 方块技巧。**

**引理 12.2**. 考虑棋盘上任意一个 2×2 方块，其四个位置中有三个放有滑块 $A, B, C$，一个为空格。空格沿该方块逆时针绕行一圈（4 步移动），效果是 $A, B, C$ 的一个 3-轮换。空格回到原来的位置。
...
**引理 12.3**. 设 $a, b, c$ 是 $\{1, \ldots, 15\}$ 中三个互不相同的位置。则 3-轮换（在位置层面）$(a\ b\ c)$ 可以通过合法操作实现（空格出发和结束都在位置 16）。

*证明*. 如果 $a, b, c$ 已经属于同一个 2×2 方块，则直接由引理 12.2 实现。

一般情况下，思路是：先用空格将位置 $a, b, c$ 上的滑块逐一"搬运"到某个固定的 2×2 方块中，在该方块中做 3-轮换，再按逆序搬回。具体地：

1. 选择一个 2×2 方块 $B$（例如 $\{9, 10, 13, 14\}$，即左下角的方块），使其与位置 16 有一条不相交于 $a, b, c$ 的路径相连。
2. 用空格将位置 $a$ 上的滑块移到方块 $B$ 的某个位置。这可以通过将空格从位置 16 移到 $a$（沿一条避开 $B$ 的路径），然后把滑块沿反方向推入 $B$ 来实现。记录这组移动为操作序列 $\rho_1$。
3. 同理，将位置 $b, c$ 上的滑块分别搬入 $B$ 的其余位置（操作序列 $\rho_2, \rho_3$）。
4. 在方块 $B$ 中用引理 12.2 执行 3-轮换（操作序列 $\omega$）。
5. 按逆序执行 $\rho_3^{-1}, \rho_2^{-1}, \rho_1^{-1}$ 将滑块搬回原位。

总操作序列为 $\rho_1 \rho_2 \rho_3\ \omega\ \rho_3^{-1} \rho_2^{-1} \rho_1^{-1}$。搬入搬出的操作精确抵消（$\rho_i^{-1}$ 是 $\rho_i$ 中操作按逆序执行，每步取逆），$\omega$ 的 3-轮换效果被正确传递到原始位置 $a, b, c$ 上的滑块。

空格在操作开始前在位置 16，在 $\rho_1$ 中移出，在 $\omega$ 后沿逆路径返回，最终仍在位置 16。$\square$

**第四步：完成充分性证明。**

设 $G$ 是空格在位置 16 时可实现的全体置换。$G$ 是 $S_{16}$ 的子群。

- 由引理 12.3，$G$ 包含所有 3-轮换（限制在 $\{1, \ldots, 15\}$ 上）。
...
可解的状态恰好占全体状态的 $|A_{16}|/|S_{16}| = 1/2$——所有可能状态中恰好一半可解，一半不可解。
...
**充分性**：2×2 方块技巧（引理 12.2）在任何 $m, n \geq 2$ 的棋盘上都可用，因此 3-轮换总可实现，充分性论证不变。
...
- **$n$ 为偶数**：$n + 1$ 为奇数，$\operatorname{sgn}(\pi) = (-1)^{\operatorname{row} - m}$。由于 $m$ 是固定的棋盘高度，条件等价于 $\operatorname{inv}(\pi) + \operatorname{row}$ 为偶数（$m$ 的奇偶性只影响等式两边同加一个常数，不改变可解状态的集合）。

- **$n$ 为奇数**：$n + 1$ 为偶数，$(-1)^{(n+1)(\operatorname{row}-m)} = 1$。因此 $\operatorname{sgn}(\pi) = +1$，条件与空格位置无关——排列 $\pi$ 本身必须是偶置换。
